...
//				        ((macPktlen + PHY_HDR_PSDU_HEADER_LENGTH)/bitrate);
				...
				double sendingDuration = RADIODELAY_11P + ((phyHeaderLength+mac->getBitLength())/bitrate)+PHY_HDR_PREAMBLE_DURATION + PHY_HDR_PLCPSIGNAL_DURATION;

				//DBG<<"phyHeaderLength:"<<phyHeaderLength<<endl;
				//DBG<<"sendingDuration:"<<sendingDuration<<endl;
				//DBG<<"duration:"<<duration<<endl;
				//DBG<<"time:"<<simTime()<<endl;
...
void Mac80211p::startTimer(t_mac_timer timer) {
	...
	if (timer == TIMER_BACKOFF) {
		...
		double sendingDuration = backOff + AIFSDelay + RADIODELAY_11P + ((phyHeaderLength+bitLength)/bitrate)+0.001;
		if (timeLeft < sendingDuration) {
		...
		double sendingDuration = AIFSDelay + RADIODELAY_11P + ((phyHeaderLength+bitLength)/bitrate)+0.001;
