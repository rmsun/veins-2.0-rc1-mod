...
void Mac1609_4::initialize(int stage) {
	...
	if (stage == 0) {
		...
		scheduleAt(simTime() + individualOffset + timeToNextSwitch,channel_switch);

//		scheduleAt(simTime() + timeToNextSwitch,channel_switch);
	...
	if (msg == channel_switch) {
		switch (csState) {
			case CCH:
				...
				DBG<<"Switching time"<<simTime()<<endl;
...
void Mac1609_4::attachAndSend(cMessage* msg, ChannelSelectorState channel, t_channel channelType, double frequency) {
    ...
    if(tmprotocolId == 0)
    {
        ...
        //  pkt->encapsulate(dynamic_cast<WaveShortMessage*>(msg));

        DBG << "Mac Sending down a SCH WAVE Packet with priority :" << priority << std::endl;

        //std::cout<<"mac04senddown:"<<sizeof(*pkt)<<endl;
        sendDown(pkt);
    }
    ...
    else if(tmprotocolId == 6)
    {
        ...
        Mac80211Pkt* pkt = new Mac80211Pkt("MacPkt including IP");
        pkt->setControlInfo(addInfo);
        pkt->addBitLength(headerLength);
        pkt->encapsulate(dynamic_cast<Ipv6Pkt*>(msg->dup()));
        //  pkt->encapsulate(dynamic_cast<WaveShortMessage*>(msg));

        DBG << "Mac Sending down a SCH IP Packet with priority :" << priority << std::endl;
     ...
     {
         EV << "mac1609_4: try to send unknown msg" << endl;
     }
...
//	std::cout<<"mac04recvup:"<<sizeof(*msg)<<endl;
    if(tmprotocolId == 0)
    {
        if (wsm != NULL)
        {
            DBG << "Received a wave message from upper layer for channel "
		    << wsm->getChannelNumber() << " Access Category (Priority):  "
		    << wsm->getPriority() << std::endl;

            //rewrite SCH channel to actual SCH the Mac1609_4 is set to
            if (wsm->getChannelNumber() == Channels::SCH1)
            {
    ...
    else if(tmprotocolId == 6)
    {
        ...
        {
            //ip没有priority信息
            DBG << "Received a ip message from upper layer for channel "
                    << ipkt->getChannelNumber() << " Access Category (Priority):  "
                    <<std::endl;

            //rewrite SCH channel to actual SCH the Mac1609_4 is set to
            if (ipkt->getChannelNumber() == Channels::SCH1)
            {
                ipkt->setChannelNumber(mySCH);
            }

            if (ipkt->getChannelNumber() == Channels::CCH)
            {
                if ((csState == CCH) && (cchQueue.empty()))
                {
                    //Send this CCH packet right away
    ...
    {
        EV << "mac1609_4: received unknown msg" << endl;
    }

}

void Mac1609_4::handleLowerMsg(cMessage* msg) {
    //std::cout<<"mac04recv:"<<sizeof(*msg)<<endl;
	sendUp(msg);
}
